    ...
    return (
        <AuthLayout title="MADYAW" subtitle="Staff Login">
            ...
            <motion.form method="post" target="_self" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} onSubmit={submit} className="space-y-4">
                ...
                <Link href={`/auth/forgot-password?role=staff&username=${encodeURIComponent(data.username || '')}${data.hotel_id ? `&hotel=${encodeURIComponent(data.hotel_id)}` : ''}`} className="text-xs text-primary hover:underline">Forgot password?</Link>
