...
export default function CategorySelection() {
    const { activeHotelId: propHotelId = '' } = usePage().props;
    const hotelId = String(propHotelId ?? '').trim();
    const hotelQuery = hotelId !== '' ? `?hotel=${encodeURIComponent(hotelId)}` : '';
    const categories = [
                ...
                <div className="w-full max-w-2xl grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    {categories.map((category, index) => (
                        <motion.div key={category.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                            {/* Full document navigation so session + hotel cookies apply reliably (same fix as hotel gate POST). */}
